...
def lines(filename, Chr):
	...
	with fn_open(filename, 'rt') as fh, open('Exac_parse_OUT.csv', 'w') as csvout:
		a = csv.writer (csvout)
		first_row = ('GENE_ID', 'TRANSCRIPT', 'TRANSCRIPT CHANGE', 'PROTEIN CHANGE', 'AA_POS', 'AA_CHANGE', 'MUTATION', 'ALLELE COUNT', 'ALLELE FREQUENCY')
					...
					for line in e.splitlines():
						if ENSP in line and "missense_variant" in line: #TODO: add deletions
							...
							if len(p_good_line['AC'])>1:
								...
								try:
									AlleleCount=sum(list(map(int,p_good_line['AC']))) #list added to break object, if iterate dont need list(
							...
							print(Variant)
							#print(':::::::::::::::::::::::::::::::::::::::::::::::')
							row_line = [match[3], match[6],match[10],match[11],match[14], match[15], ''.join(Variant),AlleleCount, AlleleFrequency]
							print (row_line)
